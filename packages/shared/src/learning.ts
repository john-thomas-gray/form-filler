import type { FillRule, FillStrategy, Rules } from "./index";
...
export function createAnswerMemory(): AnswerMemory {
  return { groups: [] };
}
...
function shouldMergeByAnswer(answer: string): boolean {
  ...
  return groups
    .map((group) => ({
      ...
      questions: group.questions.filter(
        (q) => normalizeQuestion(q) !== questionKey,
      ),
    }))
    .filter((group) => group.questions.length > 0);
}

function applyObservationMetadata(
  group: LearnedQuestionGroup,
  observation: ObservedAnswer,
): LearnedQuestionGroup {
  const next: LearnedQuestionGroup = { ...group };
...
export function recordAnswer(
  ...
  if (!question || !answer) return memory;
...
export function mergeCommunalAliases(
  memory: AnswerMemory,
  communalGroups: CommunalQuestionGroup[],
): AnswerMemory {
  return {
    groups: memory.groups.map((localGroup) => {
      const localQuestionKeys = new Set(
        localGroup.questions.map(normalizeQuestion),
      );
      const aliases = communalGroups.flatMap((communalGroup) => {
        const matchesLocalGroup = communalGroup.questions.some((question) =>
          localQuestionKeys.has(normalizeQuestion(question)),
        );
...
export function buildRulesFromMemory(memory: AnswerMemory): Rules {
  return Object.fromEntries(
    memory.groups.map((group, index): [string, FillRule] => [
      `learned_${index + 1}`,
      {
        value: group.answer,
